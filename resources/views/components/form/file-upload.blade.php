...
<div class="mb-8" id="uploader-{{ $name }}">
    <div class="border-2 border-dashed border-gray-200 rounded-lg p-6 text-center hover:border-teal-400 transition">
        ...
        <input type="file" name="{{ $name }}[]" id="{{ $name }}" multiple accept="image/*"
            class="hidden">
        <div id="{{ $name }}-file-label" class="text-xs text-gray-400 mt-2">No files selected</div>
        <div id="{{ $name }}-preview" class="mt-4 space-y-1"></div>
...
<script>
    (() => {
        ...
        const label = document.getElementById('{{ $name }}-file-label');
        ...
        function updatePreview() {
            ...
            selectedFiles.forEach((file, index) => {
                ...
                div.className =
                    'flex items-center gap-2 text-sm text-gray-600 py-1 border-b border-gray-100';
                div.innerHTML = `
                ...
                <button type="button" data-index="${index}" class="remove-file-btn text-gray-300 hover:text-red-400 transition ml-2">
