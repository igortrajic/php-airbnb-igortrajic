...
<nav class="bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between sticky top-0 z-50">
    ...
    @auth
        @if(auth()->user()->isAdmin())
            <a href="{{ route('admin.users.index') }}" class="hidden sm:inline-flex items-center px-3 py-1.5 text-sm font-medium text-emerald-800 bg-emerald-100 hover:bg-emerald-200 rounded-lg transition">
                Admin Dashboard
            </a>
        @endif
    @endauth
    ...
    <div class="flex items-center gap-4">
        ...
        @endif
@auth
    <a href="{{ route('profile.edit') }}" class="text-sm font-medium text-gray-700 hover:text-emerald-600 transition cursor-pointer">
        {{ auth()->user()->name }}
    </a>
